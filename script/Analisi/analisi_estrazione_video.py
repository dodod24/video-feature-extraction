        ...
        try:
            ...
            if predictions is not None and not predictions.empty:
                ...
                if score_col:
                    ...
                    # Se il filtro elimina tutti i frame, faccio fallback sui dati originali
                ...
                # 3.5 FILTRO MAR (Mouth Aspect Ratio) - RIMOZIONE RUMORE DA PARLATO
                mar_stats = {}
                if extract_mar:
                    mar_values = []
                    ...
                    if mar_values:
                        mar_series = pd.Series(mar_values, index=valid_preds.index)
                        mar_stats['Mean_MAR'] = mar_series.mean()
                        mar_stats['Std_MAR'] = mar_series.std()
                        if pd.isna(mar_stats['Std_MAR']): mar_stats['Std_MAR'] = 0.0
                        
                        # MASKING: Se la bocca è aperta (MAR > 0.15), escludo le AU inferiori dalle statistiche
                        mouth_aus = ['AU10', 'AU12', 'AU14', 'AU15', 'AU17', 'AU20', 'AU23', 'AU24', 'AU25', 'AU26', 'AU28']
                        mouth_aus_present = [au for au in mouth_aus if au in valid_preds.columns]
                        
                        mask_open_mouth = mar_series > 0.15
                        # Sovrascrivo con NaN. Pandas ignora in automatico i NaN quando fa le medie
                        valid_preds.loc[mask_open_mouth, mouth_aus_present] = float('nan')
                ...
                # Unisco tutti i dati statistici nel dizionario
