...
def elabora_cartella_video_iniziale(path_cartella_originale, durata_taglio=30):
    # 1. Crea il nome della nuova cartella
    path_cartella_originale = path_cartella_originale.rstrip(os.sep)
    nuova_cartella = f"{path_cartella_originale}_taglio_iniziale"
    ...
    for nome_file in os.listdir(path_cartella_originale):
        if nome_file.lower().endswith(estensioni_valide):
            ...
            output_path = os.path.join(nuova_cartella, f"{nome_base}_iniziale{estensione}")
            ...
            try:
                ...
                # IL TAGLIO PARTE DALL'INIZIO DEL VIDEO (0 secondi)
                start_time = 0
                end_time = min(durata_totale, durata_taglio)
    ...
    print("\nProcesso completato per tutti i video (Taglio Iniziale).")

# Esempio di utilizzo:
if __name__ == "__main__":
    percorso_input = "C:/Users/arion/Desktop/Tirocinio_AI_Pyt/fe/WPy64-31180/Video/CONTROLLI/records"
    elabora_cartella_video_iniziale(percorso_input)
